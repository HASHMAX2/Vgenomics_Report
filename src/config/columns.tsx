  ...
  },
  {
    ...
    columns: [
      ...
      }*/
      {
        ...
        accessorKey: "genomicAndGeneticData.ZYG",
        cell: (info) => {
          const zygosityColorMap: Record<string, string> = {
            HET: "text-black font-semibold",
            HOM: "text-[#780000] font-semibold",
          };
          const value = info.getValue<string>();
          return (
            <span className={`${zygosityColorMap[value] || "text-black"}`}>
              {value}
            </span>
          );
        },
...

      {
        // ALI
        header: "GENE",
        accessorKey: "genomicAndGeneticData.GENE",
        filterFn: filterFunctionForText,
        meta: {
          filterVariant: "text",
        },
...

      {
        ...
        cell: (info) => {
          ...
          const getSeverityColor = (severity: string) => {
            switch (severity) {
              case "High":
                return "text-[#780000] font-bold"; // Red for high severity
              case "Moderate":
                return "text-pink-600"; // Pink for moderate severity
              default:
                return "text-gray-600"; // Gray for other values
            }
          };

          return <span className={getSeverityColor(value)}>{value}</span>;
        },
        meta: {
          filterVariant: "checkbox",
        },
